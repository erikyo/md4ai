...
function displayResults(data: GeoInsightsResult) {
  ...
  const opportunitiesContainer = document.getElementById('geo-opportunities-container');
      ...
      <div class="geo-vitals-section">
        ...
        <div class="geo-vitals-grid">
          <div class="geo-vital-card">
            <div class="geo-vital-indicator ${getScoreClass(data.scores.identity_match)}">
              ${data.scores.identity_match}%
            </div>
            <div class="geo-vital-content">
              <div class="geo-vital-label">${__('Identity Match', 'md4ai')}</div>
              <div class="geo-vital-bar">
                <div class="geo-vital-bar-fill ${getScoreClass(data.scores.identity_match)}" style="width: ${data.scores.identity_match}%"></div>
              </div>
            </div>
          </div>
          <div class="geo-vital-card">
            <div class="geo-vital-indicator ${getScoreClass(data.scores.tech_match)}">
              ${data.scores.tech_match}%
            </div>
            <div class="geo-vital-content">
              <div class="geo-vital-label">${__('Technical Match', 'md4ai')}</div>
              <div class="geo-vital-bar">
                <div class="geo-vital-bar-fill ${getScoreClass(data.scores.tech_match)}" style="width: ${data.scores.tech_match}%"></div>
              </div>
            </div>
          </div>
          <div class="geo-vital-card">
            <div class="geo-vital-indicator ${getScoreClass(data.scores.ai_perception)}">
              ${data.scores.ai_perception}%
            </div>
            <div class="geo-vital-content">
              <div class="geo-vital-label">${__('AI Perception', 'md4ai')}</div>
              <div class="geo-vital-bar">
                <div class="geo-vital-bar-fill ${getScoreClass(data.scores.ai_perception)}" style="width: ${data.scores.ai_perception}%"></div>
              </div>
            </div>
          </div>
        </div>
        ...
        <!-- New GEO Metrics Row -->
        <h3 class="geo-subsection-title" style="margin-top: 20px; margin-bottom: 10px; font-size: 14px; color: #666;">${__('Detailed GEO Factors', 'md4ai')}</h3>
        <div class="geo-vitals-grid">
           <div class="geo-vital-card">
            <div class="geo-vital-indicator ${getScoreClass(data.scores.geo_structure)}">
              ${data.scores.geo_structure}
            </div>
            <div class="geo-vital-content">
              <div class="geo-vital-label">${__('Content Structure', 'md4ai')}</div>
              <div class="geo-vital-bar">
                <div class="geo-vital-bar-fill ${getScoreClass(data.scores.geo_structure)}" style="width: ${data.scores.geo_structure}%"></div>
              </div>
            </div>
          </div>
           <div class="geo-vital-card">
            <div class="geo-vital-indicator ${getScoreClass(data.scores.geo_multimedia)}">
              ${data.scores.geo_multimedia}
            </div>
            <div class="geo-vital-content">
              <div class="geo-vital-label">${__('Multimedia Usage', 'md4ai')}</div>
              <div class="geo-vital-bar">
                <div class="geo-vital-bar-fill ${getScoreClass(data.scores.geo_multimedia)}" style="width: ${data.scores.geo_multimedia}%"></div>
              </div>
            </div>
          </div>
           <div class="geo-vital-card">
            <div class="geo-vital-indicator ${getScoreClass(data.scores.geo_tech)}">
              ${data.scores.geo_tech}
            </div>
            <div class="geo-vital-content">
              <div class="geo-vital-label">${__('Tech SEO Perception', 'md4ai')}</div>
              <div class="geo-vital-bar">
                <div class="geo-vital-bar-fill ${getScoreClass(data.scores.geo_tech)}" style="width: ${data.scores.geo_tech}%"></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="geo-detailed-report">
        ...
        <div class="report-grid">
          <div class="report-item">
            <span class="report-label">${__('Website Name:', 'md4ai')}</span>
          ...
          <div class="report-item">
            ...
            <span class="report-value">${data.ground_truth.topics.join(', ')}</span>
          </div>
          <div class="report-item">
...
export function initGeoInsights() {
  const btnStart = document.getElementById('btn-start-analysis') as HTMLButtonElement;
  const loadingDiv = document.getElementById('geo-loading');
  const resultsDiv = document.getElementById('geo-results');
  const urlInput = document.getElementById('geo-url-input') as HTMLInputElement;
  const wrapper = document.querySelector('.geo-insights-wrapper') as HTMLElement;
  ...
  function validateUrl(url: string): boolean {
    try {
      ...
      return parsedUrl.hostname === siteDomain || parsedUrl.hostname.endsWith('.' + siteDomain);
    } catch {
      return false;
    }
  }
  ...
  function showErrorAiServices() {
    ...
    if (btnStart) {
      btnStart.innerHTML = `<span class="dashicons dashicons-warning"></span> ${__('AI Services Required', 'md4ai')}`;
      ...
      resultsDiv.innerHTML = `
        <div class="geo-error-message">
          ...
          <div>
            ...
            <p>${__('Install the AI Services plugin to analyze your website with AI.', 'md4ai')}</p>
            <a href="${md4aiData.blogUrl}/wp-admin/plugin-install.php?s=AI%2520Services&tab=search&type=term" target="_blank" class="geo-analyze-button">
  ...
  async function GeoInsightsInit(promptTemplate: string): Promise<void> {
    ...
    if (data.result) {
      ...
      if (btnStart) {
        btnStart.innerHTML = `<span class="dashicons dashicons-update"></span> ${__('Re-Analyze', 'md4ai')}`;
      }
    } else {
      if (resultsDiv) {
        resultsDiv.innerHTML = `
          <div class="geo-error-message">
            ...
            <div>
              ...
              <p>${__('Unable to complete the analysis. Please try again.', 'md4ai')}</p>
            </div>
          </div>
        `;
        resultsDiv.style.display = 'block';
