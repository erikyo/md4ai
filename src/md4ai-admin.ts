...
function handleMd4aiButtons() {
  ...
  const generateAiBtn = document.querySelector('.md4ai-ai-generate') as HTMLButtonElement;
  ...
  const clearMarkdown = (): void => {
    ...
    if (
      ...
      textarea.value = '';
      updateStatus(__('Custom markdown cleared.', 'md4ai'), '#46b450'); // Green color for success
  ...
  function updateMarkdown(textarea: HTMLTextAreaElement, markdown: string) {
    ...
    updateStatus(__('Markdown generated successfully!', 'md4ai'), '#46b450'); // Green color for success
  ...
  const handleGenerate = async (): Promise<void> => {
    ...
    if (!textarea) {
      console.error(`Textarea ${generateBtn.dataset.field} not found`);
      return;
    }
    ...
    try {
      ...
      if (result.markdown) {
        ...
        updateStatus(__('Error generating markdown.', 'md4ai'), '#dc3232'); // Red color for error
      }
  ...
  const handleAiGenerate = async (): Promise<void> => {
    ...
    // Get the endpoint from the dataset
    const endpoint = aiMdData.restUrl + '/' + generateAiBtn.dataset.endpoint;
    const url = aiMdData.postId ? endpoint + '/' + aiMdData.postId : endpoint;
