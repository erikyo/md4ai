...
document.addEventListener( 'DOMContentLoaded', () => {
  const generateBtn = document.getElementById('md4ai-generate') as HTMLButtonElement;
  let clearBtn = document.getElementById('md4ai-clear') as HTMLButtonElement | null;
  const textarea = document.getElementById('md4ai-textarea') as HTMLTextAreaElement;
  ...
  const handleGenerate = async (): Promise< void > => {
    ...
    try {
      ...
      const response = await fetch( `${aiMdData.restUrl}${aiMdData.postId}`, {
        method: 'POST',
        headers: {
