...
export async function md4aiCharts() {
  ...
  // --- CHART CONFIGURATION ---
  const trafficData = visitorsResponse.access ? visitorsResponse.access : null;
  ...
  if (trafficData && trafficData.source_counts) {
    ...
    new Chart(document.getElementById('md4ai-source-chart') as HTMLCanvasElement, {
      type: 'pie',
      data: {
        labels: sourceLabels,
        datasets: [{
    ...
    // Uses data from /get-visitors
    new Chart(document.getElementById('md4ai-referrals-chart') as HTMLCanvasElement, {
      type: 'bar',
      data: {
