...
- [Link title](https://link_url)';
		...
		<div class="md4ai-tabs">
			<ul class="md4ai-nav-tab-wrapper">
				...
				foreach ( $tabs as $slug => $tab ) {
					...
					printf(
						'<li class="md4ai-nav-tab tab-%s %s"><a href="%s">%s</a></li>',
						esc_attr( $slug ),
						esc_attr( $tab_active ),
	...
	public static function prepare_traffic_stats( $visitors ) {
		$source_counts           = array();
		$referral_counts_per_day = array();

		if ( empty( $visitors ) ) {
			return array(
				'source_counts'       => array(),
				'referral_chart_data' => array(
					'dates' => array(),
					'data'  => array(),
				),
				'latest_views'        => array(),
	...
	private function render_tab_dashboard() {
		$options   = get_option( MD4AI_OPTION );
		$analytics = $options['requests'] ?? array();
		$visitors  = $options['visitors'] ?? array();
